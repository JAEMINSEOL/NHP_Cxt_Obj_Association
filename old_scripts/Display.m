% figure;
% area_overall_spk_occ = vertcat(area_spk_occ, area_forest_spk_occ, area_city_spk_occ);
% Range = max( [area_overall_spk_occ] ) * 1.1;
% imagesc((area_overall_spk_occ));
% colormap(jet);
% thisAlphaZ = area_overall_spk_occ;
% thisAlphaZ(isnan(area_overall_spk_occ)) = 0;
% thisAlphaZ(~isnan(area_overall_spk_occ)) = 1;
% alpha(thisAlphaZ);
% 
% xticks([1 2 3 5 7 9 11 13 15 17 19 20])
% xticklabels({'ITI', 'Start', 'Trial 1', 'Trial 2', 'Trial 3', 'Trial 4', 'Turn', 'Trial 5', 'Trial 6', 'Trial 7', 'Trial 8', 'End'})
% 
% yticks([1 2 3])
% yticklabels({'Overall', 'Forest', 'City'})
% 
% j=1;
...
% figure;
% subplot(3,1,1)
% Range = max( [area_spk_occ] ) * 1.1;
% imagesc((area_spk_occ));
% colormap(jet);
% thisAlphaZ = area_spk_occ;
% thisAlphaZ(isnan(area_spk_occ)) = 0;
% thisAlphaZ(~isnan(area_spk_occ)) = 1;
% alpha(thisAlphaZ);
% xticks([1 2 3 5 7 9 11 13 15 17 19 20])
% xticklabels({'ITI' 'Start', 'Trial 1', 'Trial 2', 'Trial 3', 'Trial 4', 'Turn', 'Trial 5', 'Trial 6', 'Trial 7', 'Trial 8', 'End'})
% j=1;
...
% 
% subplot(3,1,2)
% Range = max( [area_forest_spk_occ] ) * 1.1;
% imagesc((area_forest_spk_occ));
% colormap(jet);
% thisAlphaZ = area_forest_spk_occ;
% thisAlphaZ(isnan(area_forest_spk_occ)) = 0;
% thisAlphaZ(~isnan(area_forest_spk_occ)) = 1;
% alpha(thisAlphaZ);
% xticks([1 2 3 5 7 9 11 13 15 17 19 20])
% xticklabels({'ITI' 'Start', 'Trial 1', 'Trial 2', 'Trial 3', 'Trial 4', 'Turn', 'Trial 5', 'Trial 6', 'Trial 7', 'Trial 8', 'End'})
% j=1;
...
% 
% subplot(3,1,3)
% Range = max( [area_city_spk_occ] ) * 1.1;
% imagesc((area_city_spk_occ));
% colormap(jet);
% thisAlphaZ = area_city_spk_occ;
% thisAlphaZ(isnan(area_city_spk_occ)) = 0;
% thisAlphaZ(~isnan(area_city_spk_occ)) = 1;
% alpha(thisAlphaZ);
% xticks([1 2 3 5 7 9 11 13 15 17 19 20])
% xticklabels({'ITI' 'Start', 'Trial 1', 'Trial 2', 'Trial 3', 'Trial 4', 'Turn', 'Trial 5', 'Trial 6', 'Trial 7', 'Trial 8', 'End'})
% j=1;
...
% figure;
% subplot(1,3,1)
% Range = max( choice_phase_overall_Trial_spk_occ ) * 1.1;
% imagesc( choice_phase_overall_Trial_spk_occ);
% colormap(jet);
% thisAlphaZ = choice_phase_overall_Trial_spk_occ;
% thisAlphaZ(isnan(choice_phase_overall_Trial_spk_occ)) = 0;
% thisAlphaZ(~isnan(choice_phase_overall_Trial_spk_occ)) = 1;
% alpha(thisAlphaZ);
% xticks([1 2 3])
% xticklabels({'Cursor Red' 'Cursor Red, Obj On' 'Cursor Blue'})
% j=1;
% minmaxColor = get(gca, 'CLim');
% if minmaxColor(1) == -1 && minmaxColor(2) == 1
%     minmaxColor(2)=0.1;
...
% 
% subplot(1,3,2)
% Range = max( choice_phase_forest_Trial_spk_occ ) * 1.1;
% imagesc( choice_phase_forest_Trial_spk_occ);
% colormap(jet);
% thisAlphaZ = choice_phase_forest_Trial_spk_occ;
% thisAlphaZ(isnan(choice_phase_forest_Trial_spk_occ)) = 0;
% thisAlphaZ(~isnan(choice_phase_forest_Trial_spk_occ)) = 1;
% alpha(thisAlphaZ);
% xticks([1 2 3])
% xticklabels({'Cursor Red' 'Cursor Red, Obj On' 'Cursor Blue'})
% j=1;
...
% 
% subplot(1,3,3)
% Range = max( choice_phase_city_Trial_spk_occ ) * 1.1;
% imagesc( choice_phase_city_Trial_spk_occ);
% colormap(jet);
% thisAlphaZ = choice_phase_city_Trial_spk_occ;
% thisAlphaZ(isnan(choice_phase_city_Trial_spk_occ)) = 0;
% thisAlphaZ(~isnan(choice_phase_city_Trial_spk_occ)) = 1;
% alpha(thisAlphaZ);
% xticks([1 2 3])
% xticklabels({'Cursor Red' 'Cursor Red, Obj On' 'Cursor Blue'})
% j=1;
...
ForestSpikePosition = vertcat(ForestOutboundSpikePosition, ForestInboundSpikePosition);
ForestSpikeTime = vertcat(ForestOutboundSpikeTime, ForestInboundSpikeTime);
ForestSpikeInfo = horzcat(ForestIndices, ForestSpikePosition, ForestSpikeTime);
...
ForestBehaviorPosition = vertcat(ForestOutboundBehaviorPosition, ForestInboundBehaviorPosition);
...
% CityInboundSpikeTimeT = (((CitySpikeTime-min(TickLog_sync_recording(:,1))))./1000000);
% 
% CityIndex = (SpikeData_info(:,3)==1);
% CityIndices = find(CityIndex==1);
% CitySpikePosition = SpikePosition(CityIndices);
% CitySpikeTime = SpikeTime(CityIndices);
% 
% CityBehaviorIndex = (TickLog_sync_recording_info(:,3)==1);
% CityBehaviorIndices = find(CityBehaviorIndex==1);
% CityBehaviorPosition = TickLog_sync_recording_info(CityBehaviorIndices,2);
% CityBehaviorTime = TickLog_sync_recording_info(CityBehaviorIndex,1);
%% Binning
Bins = 39;
BinSize = 250;
...
for h=1:Bins
    ...
    ForestOutBin{1,h} = ((((length(find((ForestOutboundSpikePosition< BinSize*h) == (ForestOutboundSpikePosition > (BinSize*(h-1))))))))./ (((length(find((ForestOutboundBehaviorPosition< BinSize*h) == (ForestOutboundBehaviorPosition > (BinSize*(h-1)))))))*(1/BinSize)));
    ForestInBin{1, h} = ((((length(find((ForestInboundSpikePosition< BinSize*h) == (ForestInboundSpikePosition > (BinSize*(h-1))))))))./ (((length(find((ForestInboundBehaviorPosition< BinSize*h) == (ForestInboundBehaviorPosition > (BinSize*(h-1)))))))*(1/BinSize)));
...
plot((ForestOutboundBehaviorPosition),(ForestOutboundBehaviorTime),'MarkerSize',0.1,'Marker','.','LineWidth',2,'LineStyle','none',...
...
xlabel('Bin (1 bin = 250cm)','FontWeight','bold','FontSize',14);
title({'Linearized firing rate on track'},'FontWeight','bold');
ylabel('Firing rate (Hz)','FontWeight','bold','FontSize',14);
axis([0 Bins 0 max(max(ForestOutArray), max(ForestInArray))*1.2]);
%
...
maxColor(j) = minmaxColor(2);
j=j+1;
text(0,13,sprintf(['Max frequency = ' num2str(ForestOutMaxFR) '(Hz)'], 'fontsize',10));
text(0,7,sprintf(['Spatial information = ' num2str(ForestOutSpaInfoScore)],'fontsize',10));
text(0,10,sprintf(['Average frequency =' num2str(ForestOutAvgFR) '(Hz)'], 'fontsize', 10));
...
maxColor(j) = minmaxColor(2);
j=j+1;
text(0,13,sprintf(['Max frequency = ' num2str(ForestInMaxFR) '(Hz)'], 'fontsize',10));
text(0,7,sprintf(['Spatial information = ' num2str(ForestInSpaInfoScore)],'fontsize',10));
text(0,10,sprintf(['Average frequency =' num2str(ForestInAvgFR) '(Hz)'], 'fontsize', 10));
